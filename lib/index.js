 ...
 *    mode=image → 附件目录（设置 attachmentDir，默认 ~/Documents/DSH/附件库，
 *    按天分文件夹）；其余 → 当前会话项目目录 uploads/（原逻辑不变）。
 *    返回绝对路径。
...
/** 附件目录默认值（用户可在设置里改） */
export const DEFAULT_ATTACHMENT_DIR = join(homedir(), 'Documents', 'DSH', '附件库')
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // 图片留档 → 附件目录（按天分文件夹）；其他文件 → 项目 uploads/（原逻辑）
        let dir
        if (mode === 'image') {
          ...
          dir = join(resolved, dayFolder())
        } else {
          dir = join(cwd, 'uploads')
        }
      ...
      try {
        ...
        if (req.method === 'GET' || req.method === 'HEAD') {
          ...
          return writeJson(res, 200, {
            ...
            maxImageBytes: limits !== undefined && typeof limits.maxImageBytes === 'number' ? limits.maxImageBytes : MAX_RAW_BYTES,
