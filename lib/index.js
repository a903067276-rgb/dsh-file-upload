...
/** 路径文本前缀默认值（输入框里加在绝对路径前，可配置为空 = 无前缀） */
export const DEFAULT_PREFIX = '[上传文件]'

/** 粘贴接管模式默认值：off=不接管（官方内建粘贴，默认关，与 allowPublic 同类），images=只接管图片，all=接管全部文件 */
export const DEFAULT_PASTE_MODE = 'off'
...
const SettingsSchema = z.object({
  ...
  prefix: z.string().required(false),
  allowPublic: z.boolean().required(false),
  pasteMode: z.union(['off', 'images', 'all']).required(false),
})
...
function readSettings(ctx) {
  ...
  return {
    ...
    prefix: typeof v.prefix === 'string' ? v.prefix : DEFAULT_PREFIX,
    allowPublic: v.allowPublic === true,
    pasteMode: v.pasteMode === 'off' || v.pasteMode === 'images' || v.pasteMode === 'all'
      ? v.pasteMode
      : DEFAULT_PASTE_MODE,
  }
}
...
// 同源校验（P1 修复，照 perm-guard 先例）：默认只放行本机页面（127.0.0.1 /
// localhost，Origin 或 Host 头其一命中即可）——防跨站页面 CSRF 式写入。
// 设置里 allowPublic=true 时放开校验，允许公网/内网穿透域名等任意来源上传
// （需在公网暴露且信任访问者的场景使用）。
function isSameOrigin(req, allowPublic) {
...
/**
 * 当前会话生效模型（provider/model）。
 * 0.1.2 起主程序改用 typert 架构，旧 apiProxy 服务已不存在（hud 同款踩坑）；
 * 模型读取顺序与官方 UI 同源（dsh-api-session-controller selectionFor 三选一）：
 *   1. sessionProjections('modelSelection').pending —— 用户刚选择的、下次请求将用
 *   2. session.requestHeader().config —— 上次实际请求的模型
 * 此后：0.1.1- 旧契约 apiProxy.sessions.models（会话级精确值）→
 *       agentDefaultModel.currentSelection()（全局默认兜底）。全部查不到 → null。
 * 各路径对服务/方法做存在性检查 + try/catch，全版本安全（0.1.1-rc.2 已包对比验证）。
 */
export async function currentModelPair(ctx, sessionId) {
  if (typeof sessionId !== 'string') return null
  let agent
  try {
    const agents = ctx.get('agents')
    agent = agents !== undefined && typeof agents.get === 'function' ? agents.get(sessionId) : undefined
  } catch (error) {
    agent = undefined
  }
  if (agent !== undefined && agent !== null) {
    ...
    if (session !== undefined && session !== null) {
      // 1) 模型选择投影（0.1.2；未注册/旧版契约缺失时 stateOf 返回 undefined）
      try {
        const projections = ctx.get('sessionProjections')
        const state = projections !== undefined && typeof projections.stateOf === 'function'
          ? projections.stateOf(session, 'modelSelection')
          : undefined
        const pending = state !== undefined && state !== null && typeof state === 'object' ? state.pending : undefined
        if (pending !== undefined && pending !== null && typeof pending.provider === 'string' && typeof pending.model === 'string') {
          return { provider: pending.provider, model: pending.model }
        }
      } catch (error) { /* 投影单元未注册等情况，走下一层 */ }
      // 2) 上次实际请求头（0.1.2）
      try {
        const header = typeof session.requestHeader === 'function' ? session.requestHeader() : undefined
        const config = header !== undefined && header !== null && typeof header === 'object' ? header.config : undefined
        if (config !== undefined && config !== null && typeof config.provider === 'string' && typeof config.model === 'string') {
          return { provider: config.provider, model: config.model }
        }
      } catch (error) { /* ignore */ }
    }
  }
  // 回退 1：0.1.1 及以前的旧契约（反映会话当前生效模型，优先于全局默认，
  // 保证旧版本上会话内切过模型时判断仍准确）
  try {
  ...
  // 回退 2：全局默认（0.1.2 selectModel 成功后会同步保存到这里；settings.yaml 同源；
  // 0.1.1 上仅当 apiProxy 也查不到时兜底）
  try {
    const agentDefaultModel = ctx.get('agentDefaultModel')
    const def = agentDefaultModel !== undefined && typeof agentDefaultModel.currentSelection === 'function'
...
/** 模型是否支持图片输入（与主程序 llm 适配器同源；查不到 = false 保守） */
export async function modelSupportsImage(ctx, provider, model) {
  const llm = ctx.get('llm')
  if (llm === undefined || typeof provider !== 'string' || typeof model !== 'string') return false
  try {
    const registration = llm.adapters !== undefined && typeof llm.adapters.get === 'function'
      ? llm.adapters.get(provider)
      : undefined
    ...
    const meta = await resolve.call(registration.adapter, provider, model)
...
/** 当前会话模型是否支持图片（查不到 = false 保守，前端走路径文本降级） */
export async function sessionImageSupported(ctx, sessionId) {
  const pair = await currentModelPair(ctx, sessionId)
  if (pair === null) return false
  return modelSupportsImage(ctx, pair.provider, pair.model)
}
...
export function apply(ctx) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      if (!isSameOrigin(req, readSettings(ctx).allowPublic)) {
      ...
      try {
        ...
        if (req.method === 'GET' || req.method === 'HEAD') {
          ...
          const supported = sessionId !== ''
            ? await sessionImageSupported(ctx, sessionId)
            : undefined
          return writeJson(res, 200, {
            ok: true,
            ...
            allowPublic: current.allowPublic,
        ...
        if (req.method === 'POST' || req.method === 'PUT') {
          ...
          if (typeof body.prefix === 'string') next.prefix = body.prefix
          if (typeof body.allowPublic === 'boolean') next.allowPublic = body.allowPublic
          if (body.pasteMode === 'off' || body.pasteMode === 'images' || body.pasteMode === 'all') next.pasteMode = body.pasteMode
  ...
  // 文件夹上传：client 用 webkitGetAsEntry 递归读出目录树（相对路径数组），
  // 这里按原结构在附件库 files/<日期>/<时间戳>-<文件夹名>/ 下重建整棵子树。
  // 每个路径段都经 sanitizeName 净化 + 拒绝 .. 片段，防目录穿越。
  ctx.effect(() => webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (entries.length === 0) {
          return writeJson(res, 400, { ok: false, error: '文件夹内容为空，无法上传' })
        }
        ...
        // 净化每一段相对路径：拒绝空段 / . / ..（非根必须折回目录名），逐段 sanitize
        const sanitizeRel = (rel) => {
          const segs = String(rel || '').split('/').filter((s) => s !== '' && s !== '.')
          if (segs.length === 0) return null
          const clean = []
          for (const seg of segs) {
            if (seg === '..') return null // 目录穿越
            const s = sanitizeName(seg)
            if (s === 'file' && seg !== 'file' && seg.trim() === '') return null
            clean.push(s)
          }
          return clean.join('/')
        }

        const rootName = `${Date.now()}-${randomUUID().slice(0, 8)}-${sanitizeName(folderName)}`
        const rootDir = join(resolved, 'files', dayFolder(), rootName)

        let totalBytes = 0
        for (const entry of entries) {
          const rel = sanitizeRel(entry.rel)
          const base64 = typeof entry.base64 === 'string' ? entry.base64 : ''
          if (rel === null || base64 === '') {
            return writeJson(res, 400, { ok: false, error: '文件夹内容无效（相对路径或文件内容异常）' })
          }
          if (!/^[A-Za-z0-9+/]+={0,2}$/.test(base64)) {
            return writeJson(res, 400, { ok: false, error: '文件内容无效' })
          }
          ...
          if (totalBytes > MAX_BASE64_BYTES) {
            return writeJson(res, 400, { ok: false, error: '文件夹过大（超过 64MB）' })
          }
        ...
        return writeJson(res, 500, { ok: false, error: '保存失败：磁盘写入错误' })
      } finally {
        inFlight -= 1
      }
    },
  }), 'dsh-file-upload.route.save-folder')
}
