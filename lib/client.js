    ...
    function apply(ctx) {
      ...
      slots.inject("conversation.input.left", () => slots.register(
        ...
        (props) => {
          ...
          // useInput 是 React hook，只能在渲染期调用（在异步回调里调用会抛错，
          // 导致后续 setDraft 全部跳过——曾实测"文件传上去了但路径没进输入框"）
          const inputState = useInput !== undefined ? useInput() : undefined;
          const currentDraft =
          ...
          const handleFiles = react.useCallback((fileList) => {
            ...
            Promise.all(files.map(saveOne)).then((results) => {
              ...
              const parts = [];
              if (paths.length > 0) parts.push(`已添加 ${paths.length} 个文件（inputActions:${inputActions === undefined ? "无" : "有"}）`);
          ...
          // stopPropagation 后官方 InputBar 不再处理（其图片拦截被绕过）
          react.useEffect(() => {
            const hasFiles = (e) =>
          ...
          return react.createElement(
            "button",
            {
              ...
              disabled: busy,
              title: "上传文件到当前项目（也可直接把文件拖进窗口）" + (inputActions === undefined ? "｜调试:inputActions缺失" : ""),
              style: {
                display: "inline-flex",
                alignItems: "center",
                gap: "4px",
