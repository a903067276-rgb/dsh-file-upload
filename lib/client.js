window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    /**
     * 组装 rc2/老宿主兜底写入器：新输入通道（insertReference/insertText/input 面）
     * 完全不可用时，退回旧 setDraft 追加——老宿主无胶囊概念，全量替换安全；
     * 0.1.2 上 input 面存在，永不走此路（其 setDraft 会抹掉已有胶囊）。
     * @param {object} inputActions 旧 slot 注入面（setDraft）
     * @param {{current: string}} draftRef 最新草稿 ref（旧 API 无 rev）
     * @param {boolean} joinNewline 路径追加用换行接续；粘贴透传原样接续
     */
    function legacyWriter(inputActions, draftRef, joinNewline) {
      if (inputActions === undefined || inputActions === null || typeof inputActions.setDraft !== "function") return null;
      return (append) => {
        const base = draftRef.current || "";
        inputActions.setDraft(joinNewline ? (base === "" ? append : base + "\n" + append) : base + append);
      };
    }

    function putPaths(sessionId, items, prefix, noticeFn, legacyAppend) {
      ...
      for (const item of items) {
        ...
        if (!chipAllowed) {
          // 老宿主兜底：input 面缺失（无 insertReference/insertText 且 caretSpan 等缺位）
          // → 旧 setDraft 追加；0.1.2 上 input 面存在，永不进此分支
          if (input === null && legacyAppend !== null) {
            legacyAppend(prefix !== "" ? `${prefix} ${item.path}` : item.path);
            continue;
          }
     ...
     */
    function pasteInsertText(sessionId, text, legacyAppend) {
      logSess = typeof sessionId === "string" ? sessionId : "";
      const input = resolveSessionInput(sessionId);
      // 老宿主兜底：input 面缺失时旧 setDraft 原样接续（粘贴透传不换行）
      if (input === null && legacyAppend !== null) {
        legacyAppend(text);
        return true;
      }
    ...
    function UploadButton(props) {
      ...
      // rc2/老宿主兜底用：仅当新输入通道完全不可用时按旧 setDraft 追加，需最新草稿做
      // base（旧 API 无 rev；与 2026-08-25 draftRef 防竞态同款：ref 每次渲染更新，
      // async 回调执行时读到最新值）。0.1.2/胶囊模式不使用（空转无害）。
      const draftRef = react.useRef("");
      draftRef.current = typeof props.input === "object" && props.input !== null && typeof props.input.draft === "string" ? props.input.draft : "";
      ...
      const handleFiles = react.useCallback((fileList) => {
        ...
        (async () => {
          ...
          if (paths.length > 0) {
            ...
            putPaths(sessionId, paths.map((p) => ({ path: p, dir: false })), prefix, showNotice, legacyWriter(inputActions, draftRef, true));
          }
          const parts = [];
      ...
      const handleFolders = react.useCallback((dirEntries) => {
        ...
        (async () => {
          ...
          if (paths.length > 0) {
            ...
            putPaths(sessionId, paths.map((p) => ({ path: p, dir: true })), prefix, showNotice, legacyWriter(inputActions, draftRef, true));
          }
          const parts = [];
      ...
      react.useEffect(() => {
        const onPaste = (e) => {
          ...
          if (text !== "") {
            pasteInsertText(sessionId, text, legacyWriter(inputActions, draftRef, false));
          }
          handleFiles(targets);
        };
